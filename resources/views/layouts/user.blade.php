<!doctype html>
<!-- Website Template by freewebsitetemplates.com -->
<html>
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Momobilan</title>
	<link rel="stylesheet" type="text/css" href="/assets/user/css/style.css">
	<link rel="stylesheet" type="text/css" href="/assets/user/css/mobile.css" media="screen and (max-width : 568px)">
	<script type="text/javascript" src="/assets/user/js/mobile.js"></script>
</head>
<body>
	<div id="header">
		<a href="index.html" class="">
			<img src="/assets/user/images/lom.jpg" alt="">
		</a>
		<ul id="navigation">
			<li class="selected">
				<a href="home">home</a>
			</li>
			<li>
				<a href="admin/mobil">Mobil</a>
			</li>
			<li>
				<a href="admin/galeri">gallery</a>
			</li>
			<li>
				<a href="blog.html">blog</a>
			</li>
			<li>
				<a href="contact.html">contact</a>
			</li>
		</ul>
	</div>
	<div id="body">
		<div id="featured">
			<img src="/assets/user/images/sport.png" alt="">
			
		</div>
		<ul>
			<li>
					<img src="../assets/user/images/audi.jpg" style="max-height:150px;max-width:150px;margin-top:7px;"/>
					<td>
							<a href="admin/pesanan" class="btn btn-success">Booking Now!</a>
					</td>
				</a>
			</li>
			<li>
					<img src="../assets/user/images/hoci.jpg" style="max-height:150px;max-width:150px;margin-top:7px;"/>
					<td>
							<a href="admin/pesanan" class="btn btn-success">Booking Now!</a>
					</td>
				</a>
			</li>
			<li>
					<img src="../assets/user/images/erti.jpg" style="max-height:150px;max-width:150px;margin-top:7px;"/>
					<td>
							<a href="admin/pesanan" class="btn btn-success">Booking Now!</a>
					</td>
				</a>
			</li>
		</ul>
	</div>
	<div id="footer">
		<div>
			<p>&copy; 2023 by Mustacchio. All rights reserved.</p>
			<ul>
				<li>
					<a href="http://freewebsitetemplates.com/go/twitter/" id="twitter">twitter</a>
				</li>
				<li>
					<a href="http://freewebsitetemplates.com/go/facebook/" id="facebook">facebook</a>
				</li>
				<li>
					<a href="http://freewebsitetemplates.com/go/googleplus/" id="googleplus">googleplus</a>
				</li>
				<li>
					<a href="http://pinterest.com/fwtemplates/" id="pinterest">pinterest</a>
				</li>
			</ul>
		</div>
	</div>
</body>
</html>
